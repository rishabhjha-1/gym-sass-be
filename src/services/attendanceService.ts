...
import { AttendanceFilter, PaginatedResponse } from '../type';
...
export class AttendanceService {
  ...
  static async getAttendanceStats(gymId: string) {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      const hourEnd = moment().startOf('day').add(i + 1, 'hours');
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const fourWeeksAgo = moment().subtract(4, 'weeks').startOf('day');
      
      const count = await prisma.$queryRaw<{ average_count: number | null }[]>`
