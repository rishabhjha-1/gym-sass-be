...
class NotificationService {
    ...
    // );
    static async sendPaymentReminder(memberId, paymentId) {
        try {
    ...
    static async sendEmailNotification(member, payment) {
        const emailBody = `
    ...
    static async sendMemberEntryNotification(gymId, member) {
        try {
    ...
    static async sendExpiredMembershipNotifications() {
        ...
        for (const membership of expiringMemberships) {
            try {
                const daysRemaining = Math.ceil((membership.endDate.getTime() - today.getTime()) / (24 * 60 * 60 * 1000));
...
NotificationService.emailTransporter = nodemailer_1.default.createTransport({
