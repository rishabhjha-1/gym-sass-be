...
app = FastAPI(title="Face Recognition Service (Light)", version="1.0.0")
...
class LightFaceRecognitionService:
    def __init__(self):
        self.face_cache = {}
        self.similarity_threshold = 0.6
        self.max_cache_size = 10  # Limit cache size to save memory
    ...
    def detect_and_encode_face(self, image: np.ndarray) -> Optional[np.ndarray]:
        """Detect face and return encoding with memory optimization"""
        try:
            ...
            # Use HOG model for faster processing and less memory
    ...
    def compare_faces(self, encoding1: np.ndarray, encoding2: np.ndarray) -> tuple[bool, float]:
        """Compare two face encodings with memory optimization"""
        try:
    ...
    def verify_face(self, image_data: str, member_id: str, stored_image_url: str) -> tuple[bool, float, float]:
        ...
        try:
            ...
            stored_encoding = self.face_cache.get(cache_key, {}).get('encoding')
            ...
            if stored_encoding is None:
                ...
                # Cache the stored encoding with timestamp
...
# Initialize service
face_service = LightFaceRecognitionService()
...
async def health_check():
    ...
    return {"status": "healthy", "service": "face-recognition-light"}
